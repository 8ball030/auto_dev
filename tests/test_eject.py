...
from auto_dev.constants import AUTO_DEV_FOLDER, DEFAULT_AUTHOR, DEFAULT_AGENT_NAME
...
def test_eject_metrics_skill_workflow(cli_runner, test_filesystem):
    ...
    wf_manager: WorkflowManager = WorkflowManager().from_yaml(file_path=Path(AUTO_DEV_FOLDER) /"data" /"workflows" / "eject_component.yaml")
    ...
    assert task.is_done and not task.is_failed, f"Task failed: {task.client.output}"
    task_2 = wf_manager.workflows[0].tasks[1].work()
    assert task_2.is_done and not task_2.is_failed, f"Task failed: {task_2.client.output}"
    assert str(Path.cwd()) == test_filesystem
...
def test_eject_metrics_skill_skip_deps(test_filesystem):
    ...
    assert task_1.is_done and not task_1.is_failed, f"Task failed: {task_1.client.output}"
    ...
    assert f"(skill, author/metrics:0.1.0)" in task_2.client.output
    assert task_2.is_done and not task_2.is_failed, f"Task failed: {task_2.client.output}"
